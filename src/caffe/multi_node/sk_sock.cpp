...
inline int SkSock::SendHeader(shared_ptr<Msg> msg) {
  ...
  memcpy(zmq_msg_data (&header_msg), header_str.data(), header_str.length()); // NOLINT
...
shared_ptr<Msg> SkSock::RecvMsg(bool blocked) {
  ...
  string header_str(string(reinterpret_cast<char *>(zmq_msg_data(&header_msg)),
...
int SkSock::Bind(const string& addr) {
  ...
  CHECK_EQ(0, rc) << "failed to bind zmq sock to " << addr;
